...
{
    const auto gate = juce::jlimit(-90.0f, -15.0f, gateThresholdDb.load());
    noiseGate.setThreshold(gate);
    ...
    inputGainLinear.setTargetValue(juce::Decibels::decibelsToGain(juce::jlimit(-24.0f, 24.0f, inputGainDb.load())));
    ...
    masterGainLinear.setTargetValue(juce::Decibels::decibelsToGain(juce::jlimit(-60.0f, 6.0f, masterDb.load())));
...
{
    const auto requested = juce::jlimit(0, 3, cabinet.load());
...
float BassAmpProcessor::saturate(float sample, float driveAmount, float characterAmount, Voicing mode) const noexcept
{
...
void BassAmpProcessor::processBlock(juce::AudioBuffer<float>& buffer, int startSample, int numSamples) noexcept
{
    ...
    {
        const auto mode = static_cast<Voicing>(juce::jlimit(0, 2, voicing.load()));
        const auto cab = static_cast<Cabinet>(juce::jlimit(0, 3, cabinet.load()));
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            const auto driveAmount = driveSmoothed.getNextValue();
            const auto characterAmount = characterSmoothed.getNextValue();
            const auto cleanAmount = lowCleanSmoothed.getNextValue();

            const auto distortedLow = saturate(low, driveAmount * 0.55f, characterAmount * 0.45f, mode);
            const auto cleanLow = low;
            const auto lowOut = distortedLow + (cleanLow - distortedLow) * cleanAmount;
            const auto highOut = saturate(high, driveAmount, characterAmount, mode);
    ...
    if (!bypassed.load())
    {
        ...
        auto subBlock = wholeBlock.getSubBlock(static_cast<size_t>(startSample), static_cast<size_t>(numSamples));
